		    ...
		    if (patk->getTarget() == 1) {
				if (Enemy[0].getHP() > 0) //判断前排精灵是否存活
					...
					if (Enemy[0].getHP() > 0) {//判断前排精灵是否存活
						cout << "要攻击谁？" << endl;
						for (int i = 1; i < 3; i++) {
							cout << i << "." << Enemy[0].getName() << " 和 " << Enemy[i].getName() << endl;
						}
						cin >> choose;
						while (choose != 1 && choose != 2) {
								cout << "错误！请重新输入！" << endl;
		    ...
		    if (patk->getTarget() == 1) {
				if (Me[0].getHP() > 0) //判断前排精灵是否存活
					...
					if (Me[0].getHP() > 0) {//判断前排精灵是否存活
						cout << "要攻击谁？" << endl;
						for (int i = 1; i < 3; i++) {
							cout << i << "." << Me[0].getName() << " 和 " << Me[i].getName() << endl;
						}
						cin >> choose;
						while (choose != 1 && choose != 2) {
								cout << "错误！请重新输入！" << endl;
...
int Fight::ProceedFight(int log, ItemBag &myItem,ElfBag &myElf)//参数标识表示是否可以捕捉和逃跑, IBag是传入背包物品数据
{
	int choose;
	int Round=1;
	Elf* patk;
...
void Fight::Turn(int n)//0-2*n-1依次代表我方n只精灵、敌方n只精灵；二维数组每个单元第一个存上述编号，第二格存对应精灵速度
{
	int t[2];
...
void Fight::Atk(Elf &atk,Elf &target)
{
	if (target.getHP() > 0) {
		if (rand() % 100 <= target.getAttr(DODGING)) {
			cout << atk.getName() << " 攻击了 " << target.getName();
			cout << "， 但是 " << target.getName() << " 躲开了攻击!" << endl;
			system("pause");
		}
		else {
			int hurt;
			hurt = atk.getAtk() - target.getDef();
			cout << atk.Name() << " 攻击了 " << target.getName() << " , ";
			if (rand() % 100 <= 0) {
				hurt *= 2;
				cout << "击中了要害! ";
			}
